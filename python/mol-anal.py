            ...
            for chain2 in model:
                for res2 in chain2:
                    if num_res2 - num_res1 > k:
                        if dist_res(res1, res2) < cutoff:
                            print chain1.name, '-', res1.num, '-', res1.name, " : ", chain2.name, '-', res2.num, '-', res2.name
                    num_res2 += 1
            num_res1 += 1
...
def dists(model):
    ...
    for chain1 in model:
        for res1 in chain1:
            ...
            for chain2 in model:
                for res2 in chain2:
                    if num_res2 - num_res1 > 0:
                        for atom1 in res1:
                            for atom2 in res2:
                                print "%s-%d-%s-%s %s-%d-%s-%s %f" % (\
                                        chain1.name, res1.num, res1.name, atom1.name, \
                                        chain2.name, res2.num, res2.name, atom2.name, \
                                        dist_atom(atom1, atom2))
                    num_res2 += 1
            num_res1 += 1
...
if __name__ == '__main__':
    par = get_par(sys.argv)
    module = par["_global"][0]
    if module == "contacts":
